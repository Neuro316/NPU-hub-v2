...
export function AITaskModal({
  ...
  }, [messages, pendingTask, justCreated])
  ...
  // ─── Voice: truly continuous, user must click to stop ───
  const createRecognition = useCallback(() => {
    ...
    recognition.onerror = (e: any) => {
      ...
      // network or not-allowed → actually stop
      stopListeningInternal()
    }
  ...
  const handleConfirm = async () => {
    ...
    if (!col) {
      setCreateError('No columns available to add task to.')
      return
    }

    // Build extra fields — only include valid DB fields
    // visibility: 'private' is stored; 'everyone' means no visibility set (team-visible by default)
    const extra: Record<string, any> = {}

    if (editFields.priority) extra.priority = editFields.priority
    if (editFields.description) extra.description = editFields.description
    if (editFields.assignee) extra.assignee = editFields.assignee
    if (editFields.due_date) extra.due_date = editFields.due_date
    if (editFields.visibility === 'private') extra.visibility = 'private'
    if (editFields.estimated_hours) extra.estimated_hours = editFields.estimated_hours
    if (editFields.raci_responsible) extra.raci_responsible = editFields.raci_responsible
    if (editFields.raci_accountable) extra.raci_accountable = editFields.raci_accountable
    if (editFields.raci_consulted) extra.raci_consulted = [editFields.raci_consulted]
    if (editFields.raci_informed) extra.raci_informed = [editFields.raci_informed]
    if (editFields.rock_tags?.length) extra.rock_tags = editFields.rock_tags

    try {
      await onCreateTask(col.id, editFields.title, extra)

      const createdTitle = editFields.title
      setPendingTask(null)
      setEditFields(null)
      setEditingTask(false)
      setJustCreated(createdTitle)

      setMessages(prev => [...prev, {
        role: 'ai',
        content: `"${createdTitle}" has been created and added to ${col.title}. Describe another task or close when done.`,
      }])
    } catch (err: any) {
      setCreateError('Failed to create task: ' + (err.message || 'Unknown error'))
    }
  }
      ...
      <div className="relative w-full max-w-xl max-h-[85vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden mx-4">
          ...
          {justCreated && (
            <div className="flex justify-center py-2">
              <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-full px-4 py-2">
                ...
                <span className="text-xs font-medium text-green-700">Created: {justCreated}</span>
              </div>
            </div>
          )}
        ...
        {pendingTask && editFields && (
          <div className="mx-4 mb-3 bg-white border border-np-blue/20 rounded-xl p-4 shadow-sm">
            ...
            <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
              ...
              <button onClick={() => { setPendingTask(null); setEditFields(null); setEditingTask(false); setCreateError(null) }}
        ...
        <div className="px-4 pb-4 pt-2 border-t border-gray-100">
          {listening && liveTranscript && (
            <div className="text-[10px] text-gray-400 italic mb-1.5 px-1 truncate">
              {liveTranscript}...
            </div>
          )}
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder={listening ? 'Listening... click mic when done' : 'Describe a task or ask me anything...'}
          ...
          <p className="text-[9px] text-gray-400 mt-1.5 text-center">
            {listening
              ? 'Recording... speak naturally, take your time. Click mic when finished.'
