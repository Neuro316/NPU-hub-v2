...
export function useTaskData() {
  ...
  const addTask = async (columnId: string, title: string, extraFields?: Partial<KanbanTask>) => {
    ...
    if (data && !error) {
      ...
      if (extraFields?.assignee) {
        logActivity(data.id, 'field_change', 'assignee', null, extraFields.assignee)
      }
      if (extraFields?.visibility === 'private') {
        logActivity(data.id, 'field_change', 'Visibility', null, 'private')
      }
    }
    return { data, error }
  }
  ...
  const updateSubtask = async (id: string, updates: Partial<Subtask>, taskId?: string) => {
    ...
    if (data && !error && taskId && 'completed' in updates) {
      logActivity(taskId, updates.completed ? 'subtask_completed' : 'subtask_uncompleted', null, null, data.title)
    }
    return { data, error }
  }
  ...
  const addComment = async (taskId: string, author: string, content: string) => {
    ...
    const { data, error } = await supabase
      ...
      .select().single()
    if (data && !error) {
      logActivity(taskId, 'comment_added', null, null, author)
    }
    return { data, error }
  }
