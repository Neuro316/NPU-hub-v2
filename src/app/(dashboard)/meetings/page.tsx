...
export default function MeetingsPage() {
  ...
  const renderMeetingCard = (m: MeetingWithAttendees) => {
    ...
    return (
      <div key={m.id}
        className="bg-white border border-gray-100 rounded-xl px-4 py-3 cursor-pointer flex items-center gap-3 mb-1.5 hover:border-gray-200 hover:shadow-sm transition-all group">
        ...
        <ChevronRight size={13} className="text-gray-300" onClick={() => router.push(`/meetings/${m.id}`)} />
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            {currentOrg?.name} &middot; {meetings.length} meeting{meetings.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200"
          ...
          <div className="w-full max-w-sm bg-white rounded-xl shadow-2xl border border-gray-100 p-5 animate-in zoom-in-95 duration-200"
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-np-dark">Delete Meeting?</h3>
                <p className="text-[10px] text-gray-400 mt-0.5 truncate max-w-[220px]">{deleteTarget.title}</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-lg px-3 py-2.5 mb-4 space-y-1">
              <p className="text-[10px] text-gray-500">&#8226; The prev/next meeting chain will be repaired automatically</p>
              <p className="text-[10px] text-gray-500">&#8226; Deferred action items will carry forward to the next linked meeting</p>
              <p className="text-[10px] text-gray-500">&#8226; Approved tasks in Task Manager are not affected</p>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteTarget(null)} className="px-3 py-1.5 text-xs text-gray-400 hover:text-np-dark">Cancel</button>
