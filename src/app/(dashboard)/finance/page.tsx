...
function Btn({ children, variant = 'primary', size = 'sm', className = '', ...p }: React.ButtonHTMLAttributes<HTMLButtonElement> & {
  ...
  const sz = size === 'xs' ? 'px-2.5 py-1.5 text-xs' : 'px-3 py-2 text-sm'
  const v  = { primary: 'bg-np-blue text-white hover:bg-np-blue/90', secondary: 'bg-gray-100 text-np-dark hover:bg-gray-200', ghost: 'text-gray-500 hover:text-np-dark hover:bg-gray-100', danger: 'text-red-600 hover:bg-red-50' }[variant]
  return <button {...p} className={`inline-flex items-center gap-1.5 font-medium rounded-lg transition-colors disabled:opacity-50 ${sz} ${v} ${className}`}>{children}</button>
}
