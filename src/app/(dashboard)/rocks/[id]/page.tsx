...
import {
  ChevronLeft, Plus, Clock, Loader2, Target,
    ...
    <div className="flex gap-0.5">
      {active.map(r => (
        <span key={r.key} className="text-[8px] font-bold w-3.5 h-3.5 rounded flex items-center justify-center text-white"
          style={{ background: r.color }} title={`${r.label}: ${fields[r.key]}`}>{r.label}</span>
      ))}
    </div>
  )
}
...
function RaciForm({ raci, teamMembers, onChange }: {
  ...
  const roles = [
    { key: 'responsible', label: 'Responsible', color: '#2563EB' },
    { key: 'accountable', label: 'Accountable', color: '#DC2626' },
    { key: 'consulted', label: 'Consulted', color: '#D97706' },
    { key: 'informed', label: 'Informed', color: '#6B7280' },
  ]
  return (
...
function AiAdvisorPanel({ analysis, loading, error, onAddTask, onSaveDeps, depsLoading, onClose, onRetry }: {
  analysis: RockAnalysis | null; loading: boolean; error: string | null
  onAddTask: (title: string, priority: string, raci: Record<string, string | null>) => void
  onSaveDeps: (deps: RockAnalysis['dependencies']) => void
  depsLoading: boolean; onClose: () => void; onRetry: () => void
}) {
  if (loading) {
    return (
      <div className="bg-violet-50 border border-violet-200 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold text-violet-600">AI Analyzing Rock...</span>
        </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-red-700">AI Advisor Error</span>
          </div>
          <button onClick={onClose} className="p-1 rounded hover:bg-red-100"><X size={12} className="text-red-400" /></button>
        </div>
        <p className="text-xs text-red-600 mt-2">{error}</p>
        <button onClick={onRetry}
          className="mt-3 flex items-center gap-1 px-3 py-1.5 text-[10px] font-semibold text-red-600 bg-red-100 hover:bg-red-200 rounded-lg">
          <Brain size={10} /> Try Again
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between bg-violet-50">
        ...
        <button onClick={onClose} className="p-1 rounded hover:bg-violet-100"><X size={12} /></button>
      </div>

      <div className="p-5 space-y-4">
        {/* Strategic Insight */}
        <div className="bg-violet-50 border border-violet-100 rounded-lg p-3">
          <span className="text-[10px] font-bold text-violet-600 uppercase tracking-wider">Strategic Insight</span>
          ...
          <div>
            ...
            <div className="space-y-1">
              {analysis.dependencies.map((dep, i) => (
                <div key={i} className="flex items-center gap-2 text-[11px] p-2 bg-gray-50 rounded-lg">
...
export default function RockDetailPage() {
  ...
  const loadTasks = useCallback(async () => {
    ...
    const { data: cols } = await supabase
      .from('kanban_columns').select('id, title, sort_order').eq('org_id', currentOrg.id).order('sort_order')
    if (cols) {
    ...
    const { data } = await supabase.from('kanban_tasks').select('*').eq('rock_id', id).order('sort_order')
  ...
  const getTaskStatus = (t: KanbanTask): 'done' | 'todo' => doneColumnIds.has(t.column_id) ? 'done' : 'todo'
  ...
  const handleAddTask = async (title?: string, priority?: string, raci?: Record<string, string | null>) => {
    ...
    if (!assigneeId && raciData.responsible) {
      const match = members.find(m => m.display_name?.toLowerCase().includes(String(raciData.responsible).toLowerCase()))
    ...
    const { error } = await supabase.from('kanban_tasks').insert({
      org_id: currentOrg.id, column_id: defaultColumnId, title: taskTitle,
    ...
    if (!error) {
      ...
      loadTasks(); refetchRocks()
    }
  }

  const handleStatusChange = async (newStatus: RockStatus) => {
    if (!rock) return
    await updateRock(rock.id, { status: newStatus }); refetchRocks()
  }

  const handleOwnerChange = async (newOwnerId: string) => {
    if (!rock) return
    await updateRock(rock.id, { owner_id: newOwnerId || null }); refetchRocks(); setEditingOwner(false)
  }
  ...
  const handleTaskAssigneeChange = async (taskId: string, assigneeId: string) => {
    ...
    loadTasks(); setEditingTaskAssignee(null)
  }
  ...
  const saveDependencies = async (deps: RockAnalysis['dependencies']) => {
    ...
    try {
      for (const dep of deps) {
        const targetRock = rocks.find(r => r.title.toLowerCase() === dep.rock_title.toLowerCase() && r.id !== rock.id)
        if (!targetRock) continue
        await supabase.from('rock_dependencies').upsert({
          org_id: currentOrg.id, source_rock_id: rock.id, target_rock_id: targetRock.id,
          relationship: dep.relationship, notes: dep.explanation,
        }, { onConflict: 'source_rock_id,target_rock_id' })
      }
    } catch (e) { console.error('Save deps error:', e) }
    setDepsLoading(false)
  }

  const runAiAdvisor = async () => {
    if (!rock) return
    setShowAdvisor(true); setAdvisorLoading(true); setAdvisorError(null); setAnalysis(null)
    try {
      const taskData = tasks.map(t => ({
        title: t.title, priority: t.priority, done: doneColumnIds.has(t.column_id),
        assignee_name: members.find(m => m.user_id === t.assignee)?.display_name || null,
      }))
      const res = await fetch('/api/ai/rock-advisor', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          rock: { title: rock.title, description: rock.description, status: rock.status,
            owner_name: rock.owner_name, progress_pct: rock.progress_pct, due_date: rock.due_date },
          allRocks: rocks.map(r => ({ title: r.title, status: r.status, progress_pct: r.progress_pct, owner_name: r.owner_name })),
          existingTasks: taskData,
          teamMembers: members.map(m => ({ display_name: m.display_name, job_title: (m as any).job_title || '', role: m.role })),
        }),
      })
      if (!res.ok) {
      ...
      setAdvisorError(e.message || 'Failed to analyze rock. Check that ANTHROPIC_API_KEY is set in environment variables.')
    }
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <select value={rock.status} onChange={e => handleStatusChange(e.target.value as RockStatus)}
                ...
                {members.filter(m => m.user_id).map(m => (
                  <button key={m.user_id} onClick={() => handleOwnerChange(m.user_id as string)}
                    className={`w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 flex items-center gap-2 ${
                      rock.owner_id === m.user_id ? 'text-np-blue font-semibold bg-blue-50' : 'text-np-dark'
                    }`}>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <button onClick={runAiAdvisor} disabled={advisorLoading}
            ...
            {advisorLoading ? <Loader2 size={16} className="animate-spin" /> : <Brain size={16} />}
            {advisorLoading ? 'Analyzing...' : 'Run AI Rock Advisor'}
          </button>
          <p className="text-[10px] text-gray-400 text-center mt-1.5">Analyzes dependencies, risks, and recommends tasks with RACI assignments</p>
        </div>
      </div>
      ...
      {showAdvisor && (
        <AiAdvisorPanel
          analysis={analysis} loading={advisorLoading} error={advisorError}
          onAddTask={(title, priority, raci) => handleAddTask(title, priority, raci)}
          onSaveDeps={saveDependencies} depsLoading={depsLoading}
          onClose={() => { setShowAdvisor(false); setAnalysis(null); setAdvisorError(null) }}
          onRetry={runAiAdvisor}
        />
      )}
        ...
        <div className="flex gap-1.5 mb-3">
          {(['all', 'remaining', 'done'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'all' ? `All (${tasks.length})` : f === 'remaining'
                ? `Remaining (${tasks.filter(t => !doneColumnIds.has(t.column_id)).length})`
                : `Done (${tasks.filter(t => doneColumnIds.has(t.column_id)).length})`}
            </button>
          ))}
        </div>
          ...
          <div className="mb-3 p-3 bg-gray-50 rounded-lg border border-gray-100 space-y-2">
            ...
            <div className="flex items-center gap-2">
              <select value={newTaskPriority} onChange={e => setNewTaskPriority(e.target.value)}
                className="px-2 py-1.5 text-[10px] border border-gray-200 rounded-md">
                <option value="low">Low</option><option value="medium">Medium</option>
                <option value="high">High</option><option value="urgent">Urgent</option>
              </select>
              <select value={newTaskAssignee} onChange={e => setNewTaskAssignee(e.target.value)}
                className="px-2 py-1.5 text-[10px] border border-gray-200 rounded-md flex-1">
              ...
              <button onClick={() => setShowRaci(!showRaci)}
                className={`px-2 py-1.5 text-[10px] font-semibold rounded-md border transition-colors ${
                  showRaci ? 'bg-violet-50 text-violet-600 border-violet-200' : 'bg-gray-50 text-gray-500 border-gray-200'
                }`}>RACI</button>
              <button onClick={() => handleAddTask()} disabled={!newTaskTitle.trim()}
                className="px-3 py-1.5 bg-np-blue text-white text-[10px] font-semibold rounded-md disabled:opacity-50">Add</button>
        ...
        <div>
          {filteredTasks.map(t => {
            ...
            return (
              <div key={t.id} className="flex items-center gap-2 py-2 border-b border-gray-100/70 last:border-0 group">
                ...
                <div className="relative" onClick={e => e.stopPropagation()}>
                  <button onClick={() => setEditingTaskAssignee(editingTaskAssignee === t.id ? null : t.id)}
                    ...
                    {initials ? <Avatar initials={initials} size={20} /> : (
                      <div className="w-5 h-5 rounded-full border-2 border-dashed border-gray-200 flex items-center justify-center opacity-0 group-hover:opacity-100">
                    ...
                    <div className="absolute top-full right-0 mt-1 bg-white border border-gray-100 rounded-lg shadow-lg z-20 py-1 min-w-[160px]">
                      ...
                      {members.filter(m => m.user_id).map(m => (
                        <button key={m.user_id} onClick={() => handleTaskAssigneeChange(t.id, m.user_id as string)}
                          className={`w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 flex items-center gap-2 ${
                            t.assignee === m.user_id ? 'text-np-blue font-semibold bg-blue-50' : 'text-np-dark'
                          }`}>
                ...
                <button onClick={() => router.push(`/tasks?task=${t.id}`)} title="Open in Task Manager"
          ...
          {filteredTasks.length === 0 && (
            <p className="text-xs text-gray-400 text-center py-6">
              {filter === 'all' ? 'No tasks linked. Add one above or use AI Advisor.' : `No ${filter} tasks.`}
            </p>
          )}
        </div>
      </div>
