...
import { Plus, Filter, Search, CheckSquare, Inbox, Sparkles, X, Bot, Loader2 } from 'lucide-react'
...
function TasksPageInner() {
  ...
  // ─── Text search ───
  const textFilteredTasks = useMemo(() => {
    let filtered = tasks
    ...
    return filtered
  }, [tasks, filterMember, searchQuery, aiSearchResults])
  ...
  const handleAISearch = async () => {
    ...
    try {
      ...
      if (data.task_ids) {
        ...
        // AI couldn't find structured results, show message
        setAiSearchResults([])
      }
    } catch {
      setAiSearchResults([])
  ...
  const handleQuickAddTask = async () => {
    ...
    const targetCol = newTaskColumn
      ...
      : sortedCols[0]
    if (!targetCol) return
    await addTask(targetCol.id, newTaskTitle.trim(), { assignee: newTaskAssignee || currentUser, description: newTaskNotes.trim() || undefined })
    setNewTaskTitle('')
    setAddingTask(false)
    setNewTaskColumn(''); setNewTaskAssignee(''); setNewTaskNotes('')
  }
    ...
    <div>
      ...
      <div className="mb-4 bg-white border border-gray-200 rounded-xl p-3">
        <div className="flex items-center gap-2">
          ...
          {isSearchActive && (
            <button onClick={clearSearch} className="text-[10px] text-gray-400 hover:text-np-dark font-medium px-2">
              Clear
            </button>
          )}
        </div>
      ...
      {addingTask && (
        <div className="mb-4 bg-white border border-gray-200 rounded-xl p-4 max-w-md">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { setAddingTask(false); setNewTaskTitle(''); setNewTaskColumn(''); setNewTaskAssignee(''); setNewTaskNotes('') }} className="btn-secondary text-xs py-1.5 px-4">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {sortedCols.map(col => (
          <KanbanColumnView
            key={col.id}
            column={col}
            tasks={textFilteredTasks.filter(t => t.column_id === col.id)}
            teamMembers={teamMemberNames}
            colorOverrides={colorOverrides}
