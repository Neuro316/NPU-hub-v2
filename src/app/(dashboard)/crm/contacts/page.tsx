...
export default function ContactsPage() {
  ...
  const [bulkFeedback, setBulkFeedback] = useState<{ type: 'success' | 'error'; msg: string } | null>(null)
  ...
  const handleBulkAction = async (action: string, params: Record<string, unknown>) => {
    ...
    try {
      ...
      if (result.success && result.affected > 0) {
        ...
        setBulkFeedback({ type: 'error', msg: result.error || 'No contacts were updated. Check org membership.' })
      } else if (!result.success) {
        setBulkFeedback({ type: 'error', msg: result.error || 'Update failed' })
      }
      setTimeout(() => setBulkFeedback(null), 5000)
    } catch (e: any) {
      console.error('Bulk action error:', e)
      setBulkFeedback({ type: 'error', msg: e.message || 'Bulk action failed' })
      setTimeout(() => setBulkFeedback(null), 5000)
    }
  }
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-np-blue/5 border border-np-blue/20 rounded-lg flex-wrap">
          ...
          {bulkFeedback && (
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
              bulkFeedback.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
            }`}>{bulkFeedback.msg}</span>
          )}
