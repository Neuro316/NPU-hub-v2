  ...
  const executeImport = async () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      try {
        ...
        // Normalize outreach_status to valid enum values
        const normalizeOutreachStatus = (v: string): string | null => {
          const map: Record<string, string> = {
            'prospect': 'not_started', 'new': 'not_started', 'not started': 'not_started',
            'not_started': 'not_started',
            'contacted': 'contacted', 'outreach sent': 'contacted', 'dm sent': 'contacted',
            'responded': 'responded', 'replied': 'responded', 'response received': 'responded',
            'in relationship': 'in_relationship', 'in_relationship': 'in_relationship',
            'active': 'in_relationship', 'ongoing': 'in_relationship',
            'partner': 'partner', 'signed': 'partner', 'confirmed': 'partner',
            'not a fit': 'not_a_fit', 'not_a_fit': 'not_a_fit',
            'no fit': 'not_a_fit', 'declined': 'not_a_fit', 'rejected': 'not_a_fit',
          }
          return map[v.toLowerCase().trim()] || 'not_started'
        }
        if (row.outreach_status) {
          contact.outreach_status = normalizeOutreachStatus(row.outreach_status)
        }
        ...
        const directFields = [
          ...
          'outreach_owner', 'outreach_strategy',
